...
const GoogleSignInButton = ({ onSuccess, onError }) => {
  ...
  const [googleButtonLoaded, setGoogleButtonLoaded] = React.useState(false);
  ...
  return (
    <div>
      <div ref={buttonRef} style={{ width: '100%', display: googleButtonLoaded ? 'block' : 'none' }}></div>
      
      {/* Fallback button only shows if Google button doesn't load */}
      {!googleButtonLoaded && (
        <button
